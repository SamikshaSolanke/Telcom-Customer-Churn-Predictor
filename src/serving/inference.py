...
try:
    ...
    try:
        ...
        import glob
        # Search recursively for MLmodel files in mlruns
        mlmodel_files = glob.glob("./mlruns/**/MLmodel", recursive=True)
        if mlmodel_files:
            latest_file = max(mlmodel_files, key=os.path.getmtime)
            latest_model = os.path.dirname(latest_file)
...
try:
    ...
    if not os.path.exists(feature_file):
        # Fallback to local artifacts directory in workspace
        ...
        if os.path.exists(feature_file):
            ...
            # Fallback to recursively searching in mlruns
            import glob
            feature_txt_files = glob.glob("./mlruns/**/feature_columns.txt", recursive=True)
            if feature_txt_files:
                latest_txt = max(feature_txt_files, key=os.path.getmtime)
                with open(latest_txt) as f:
                    FEATURE_COLS = [ln.strip() for ln in f if ln.strip()]
            else:
                raise FileNotFoundError("Could not find feature_columns file in model dir, local artifacts, or mlruns")
    else:
        with open(feature_file) as f:
            FEATURE_COLS = [ln.strip() for ln in f if ln.strip()]
